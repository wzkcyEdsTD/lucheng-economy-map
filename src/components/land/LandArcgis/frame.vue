...
<div class="EcoFrame" :style="{left:Left +'px'}">
    ...
    <div class="title">
      ...
      <div class="info">
        <div class="info_content">
          ...
          <div class="jyfw">
            <head>经营范围</head>
            <p>{{company.length ? company[0].yjfw : ""}}</p>
          </div>
        </div>
      </div>
    </div>
    <span class="_line"/>
    <div class="list" v-if="rquota">
      <ul class="xxtitle">
        <li class="xxtitle_li"
  ...
  <span class="_line" />
  <bigImg v-if="showImg" @clickit="viewImg" :imgSrc="`${server}/icon/tsjjImg/${this.name}.png`"></bigimg>
</div>
</template>
...
import bigImg from "@/components/economy/EcoArcgis/bigImg";
export default {
  name: "Ecoframe",
  data() {
    return {
        ...
        this.test = {
          ...
          用地信息: {
            ...
            d: rent.map(item => {
              return [
                item.czqy || "-",
                item.zyydarea ? parseFloat(item.zyydarea).toFixed(2) : "-"
              ];
            })
          }
        };
